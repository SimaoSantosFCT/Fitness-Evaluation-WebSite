...
export default function App() {
    ...
    const goToNewEvaluation = (clientData) => {
    ...
    const navBtn = (id, label, icon) => (
        <button
            onClick={() => { setPage(id); if (id === 'evaluate') setPrefill(null) }}
            style={{
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <main style={{ flex: 1 }}>
                {page === 'evaluate' && <EvaluationPage prefill={prefill} />}
                {page === 'history'  && <HistoryPage onNewEvaluation={goToNewEvaluation} />}
            </main>
        </div>
    )
}