...
export default function EvaluationPage({ prefill }) {
  ...
  // Quando chegam dados pré-preenchidos do histórico, aplica-os
  useEffect(() => {
    if (!prefill) return
    setForm({
      ...
      height:          String(prefill.height || ''),
      // Peso e dados variáveis ficam em branco para a Magda preencher
      peso:            '',
      fatMass:         '',
      boneMass:        '',
      water:           '',
      ...
      basalMetabolism: '',
      metabolicAge:    '',
      physicalLevel:   String(prefill.physicalLevel || '5'),
          ...
          <form onSubmit={handleSubmit} style={{ padding: '28px 28px 40px' }}>
            ...
            {prefill ? (
                <p style={{ fontSize: 12, color: 'var(--gold)', marginBottom: 4, fontWeight: 500 }}>
                  ✦ Dados fixos pré-preenchidos — preenche os valores medidos hoje.
                </p>
            ) : (
